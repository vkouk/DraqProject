...
jQuery(document).ready(function($) {
	...
	$('.dropdown').hover(function() {
		$(this).children('.nav-subtab').stop(true, false, true).slideToggle(300);
	});

    questionnaire();

    if (finalRiskFactor!=undefined && Number.isInteger(finalRiskFactor))
    {
        showRiskfactor(finalRiskFactor);
    }
});
...
function showRiskfactor(risk)
{
   var selector = '.result-' + risk  ;
    $(selector).addClass('scaleAnimation');
}